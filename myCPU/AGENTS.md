...
- 同日后续已把第一组可回归 workload 观测收窄到 `vector_cnn_smoke`：pipeline vector CNN 现在必须暴露非零 RAM `shadow_cache` 信号，且全局 shadow-cache 统计要与 RAM region 级统计一致。随后又把 `functional` backend 的最小 `ExecutionProfile` 观测接到 debug snapshot，并继续纳入 `LR/SC/AMO` atomic traffic，默认 `run-workload-xv6` 现在也会稳定导出更完整的非零 `profile / shadow-cache`；`xv6_boot_smoke` 已锁住当前 5000-step functional baseline 与 RAM region 一致性，`test-host-run_debug_cli_probe` 也已锁住真实 probe summary 出口。当前 stable shadow-cache baseline 仍以 pipeline vector CNN 为准，`xv6` 则转为 stable functional observation path。
