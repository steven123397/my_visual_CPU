...
- 同日后续已把第一组可回归 workload 观测收窄到 `vector_cnn_smoke`：pipeline vector CNN 现在必须暴露非零 RAM `shadow_cache` 信号，且全局 shadow-cache 统计要与 RAM region 级统计一致。随后又把 `functional` backend 的最小 `ExecutionProfile` 观测接到 debug snapshot，默认 `run-workload-xv6` 现在也会稳定导出非零 `profile / shadow-cache`；当前 stable shadow-cache baseline 仍以 pipeline vector CNN 为准，`xv6` 则转为 stable functional observation path。
...
8. `P4-prep-1` 与 `C1 / P4-prep-2 memory observation / shadow cache` 第一刀当前已经完成；如果后续继续评估 `Phase 4`，应先用现有 `profile.shadow_cache` 观测积累 workload 证据。当前第一组稳定 shadow-cache baseline 是 pipeline vector CNN；默认 `run-workload-xv6` 也已具备 stable functional observation path。后续若要把 `xv6 / Linux` 变成更像未来 cache 评估的 pipeline-side workload baseline，仍需先单独收口 pipeline bring-up gap，不直接跳到真实 `cache / DMA / multicore`。
