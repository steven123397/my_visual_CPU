...
- `UART16550` 当前已补齐一组更贴近 `xv6/Linux 8250` 的最小 16550 contract：`LCR/FCR/DLAB`、`DLL/DLM` divisor latch 访问、`MCR/MSR`、RX-ready / TX-THRE `IIR` 身份、TX 写后 `LSR.THRE/TEMT` 先拉低并在下一次 `tick()` 后恢复，以及 RX/TX pending 驱动的 PLIC source 断言。
...
- 同日后续已把第一组可回归 workload 观测收窄到 `vector_cnn_smoke`：pipeline vector CNN 现在必须暴露非零 RAM `shadow_cache` 信号，且全局 shadow-cache 统计要与 RAM region 级统计一致。随后又把 `functional` backend 的最小 `ExecutionProfile` 观测接到 debug snapshot，并继续纳入 `LR/SC/AMO` atomic traffic，默认 `run-workload-xv6` 现在也会稳定导出更完整的非零 `profile / shadow-cache`；`xv6_boot_smoke` 已锁住当前 5000-step functional baseline 与 RAM region 一致性，`test-host-run_debug_cli_probe` 也已锁住真实 probe summary 出口，这两条 guardrail 现已进入默认 `make test` / `make test-pipeline`。同一 probe target 现在还锁住了一组不依赖外部 Linux `Image` 的 `linux_proto` functional observation baseline：repo-local dummy payload 经过 `linux_sbi_shim + dtb/initrd` 合同后，会稳定导出非零 `profile / shadow-cache`。当前 stable shadow-cache baseline 仍以 pipeline vector CNN 为准，`xv6` 则转为 stable functional observation path。
...
- [tests/host/xv6_boot_smoke.cpp](tests/host/xv6_boot_smoke.cpp)、[tests/host/xv6_shell_smoke.cpp](tests/host/xv6_shell_smoke.cpp) 和 [workloads/boards/mycpu_virt.mk](workloads/boards/mycpu_virt.mk)
  `xv6` 当前已经切到真实 `virtio-blk` board profile，并已稳定到 shell；`xv6_boot_smoke` 现在除了守住 early-boot checkpoint，也会锁住 functional 5000-step `profile / shadow-cache` baseline。当前更值钱的下一步不再是继续证明 `xv6` 自身能否到 shell，而是把这条真实 board path 守成稳定 guardrail，并在此基础上把真实 Linux 从当前 `devtmpfs: initialized` / `Unpacking initramfs...` / `xor` checkpoint 继续推进到 `devtmpfs: mounted`、rootfs 和 `init` 阶段。
...
6. 继续用 `make test`、`make test-pipeline`、loader 单测、`debug_cli_smoke`、`interactive_terminal_smoke`、`virtio_blk_smoke`、已提升进默认门禁的 `run_debug_cli_probe` / `xv6_boot_smoke`、`xv6_shell_smoke`、`run-workload-xv6` 和 guest 正负回归守住当前稳定基线，不让 `pipeline` 与调试链路反向污染 reference path。
...
8. `P4-prep-1` 与 `C1 / P4-prep-2 memory observation / shadow cache` 第一刀当前已经完成；如果后续继续评估 `Phase 4`，应先用现有 `profile.shadow_cache` 观测积累 workload 证据。当前第一组稳定 shadow-cache baseline 是 pipeline vector CNN；默认 `run-workload-xv6`、`xv6_boot_smoke` 与 repo-local `linux_proto` dummy-payload probe 也已具备 stable functional observation path。后续若要把 `xv6 / Linux` 变成更像未来 cache 评估的 pipeline-side workload baseline，仍需先单独收口 pipeline bring-up gap，不直接跳到真实 `cache / DMA / multicore`。
