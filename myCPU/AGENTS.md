...
9. `NPU / TPU-like` AI accelerator 的 wave 1 已继续落地到任务 5：当前已有 `DMA-ready` memory contract、静态 graph package / tensor golden model、独立 `MMIO` AI accelerator 控制面 / submission-completion queue / PLIC interrupt / debug snapshot 骨架、独立 `scratchpad + DMA/load-store engine`，以及第一版静态子图调度器与代表性 compute path。当前 `AiAccelerator` 已支持 `DMA load -> static graph compute -> DMA store` 的异步数据面状态机，并可稳定暴露 `device_cycles / dma_cycles / compute_cycles / stall_cycles / retired_ops`；这一版 `timed-simple` 仍明确采用 `DMA + compute` 不重叠的保守语义，后续应先收口 host/profile 入口，不要把这条线反向混入 CPU ISA reference path。
