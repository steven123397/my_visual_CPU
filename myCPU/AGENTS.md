...
9. `NPU / TPU-like` AI accelerator 的 wave 1 已开始落地：当前已有 `DMA-ready` memory contract、静态 graph package / tensor golden model，以及独立 `MMIO` AI accelerator 控制面 / submission-completion queue / PLIC interrupt / debug snapshot 骨架。当前这条线还没有真实时序模型，性能口径后续应统一落在 `timed-simple` 的 `simulated cycles`，而不是宿主机 wall-clock；实施顺序仍应先补 scratchpad 与 DMA/load-store engine 的 timing，再接调度器和代表性 compute path，不要把这条线反向混入 CPU ISA reference path。
