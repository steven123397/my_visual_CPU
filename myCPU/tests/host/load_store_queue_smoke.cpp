...
int main() {
    ...
    if (!expect(load_entry.has_value() && load_entry->kind == LsqEntryKind::Load && load_entry->address_ready &&
                    load_entry->address == 0x1000 && !load_entry->data_ready,
                "LSQ should track address readiness for load entries")) {
        return 1;
    }
    ...
    if (!expect(!lsq.retire_store(load).has_value(),
                "retire_store should ignore non-store indices")) {
        return 1;
    }

    const auto retired_store = lsq.retire_store(store);
    if (!expect(retired_store.has_value() && retired_store->sequence_id == 2 && lsq.size() == 1,
                "retire_store should remove a ready store entry from the queue")) {
        return 1;
    }
    ...
    if (!expect(lsq.size() == 1 && lsq.peek(load).has_value(),
