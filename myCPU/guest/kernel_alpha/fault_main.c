...
typedef struct KernelAlphaFaultState {
    vm_address_space_t* kernel_address_space;
} kernel_alpha_fault_state_t;
...
static bool kernel_alpha_fault_setup_vm(kernel_alpha_fault_state_t* state) {
...
void kernel_main(void) {
    ...
    kernel_alpha_fault_state_t state = {0};
    ...
    if (!kernel_alpha_fault_setup_vm(&state)) {
        panic_shutdown();
    }
    console_putc('V');
