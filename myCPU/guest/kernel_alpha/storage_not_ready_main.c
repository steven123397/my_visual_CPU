...
void kernel_main(void) {
    ...
    storage_clear_error();
    if (!storage_read_info(&storage_info_after_clear) ||
        (storage_info_after_clear.status & STORAGE_STATUS_ATTACHED) == 0 ||
        (storage_info_after_clear.status & STORAGE_STATUS_READY) != 0 ||
        (storage_info_after_clear.status & STORAGE_STATUS_ERROR) != 0 ||
