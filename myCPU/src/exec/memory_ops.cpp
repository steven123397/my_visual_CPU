...
bool apply_memory_effects(CPU& cpu, Bus& bus, const InsnEffects& effects) {
    ...
    case MemoryRequest::Kind::Load: {
        ...
        if (!result.ok) {
            if (result.fault.valid) {
                cpu.trap().enter_exception(result.fault.cause, result.fault.tval);
            }
            return false;
        }
        cpu.core().write_gpr(effects.mem.rd, extend_loaded_value(result.value, effects.mem.size, effects.mem.sign_extend));
        return effects.retired;
    }
    case MemoryRequest::Kind::Store: {
        const AddressSpace::AccessResult result =
            cpu.address_space().store_result(bus, effects.mem.addr, effects.mem.store_value, effects.mem.size);
        if (!result.ok) {
            if (result.fault.valid) {
                cpu.trap().enter_exception(result.fault.cause, result.fault.tval);
            }
