...
> **文档状态：** 执行中，任务 1 已完成（`2026-04-23` 已完成方向 design / status / wave 1 plan；同日已完成任务 1：`DMA-ready` memory contract）
>
...
- [ ] **步骤 1：** 固定 graph package 的最小 header、tensor table、op descriptor、dependency 与 memory plan 编码。
- [ ] **步骤 2：** 定义 reject matrix：非法 dtype、非法 rank / tile、越界 dependency、非法 scratchpad budget、未知 op。
- [ ] **步骤 3：** 实现代表性 golden ops：
  - `INT8 / INT16 -> INT32 accumulate` 的 `gemm / conv`
  - `FP16 / BF16 -> FP32 accumulate` 的 `gemm`
  - `eltwise / pool / reduce / layout transform`
- [ ] **步骤 4：** 用 fixed tensor vectors 和小矩阵样本建立可重复对拍数据，不依赖随机值作为主门禁。
- [ ] **步骤 5：** 运行：
...
- [ ] **步骤 1：** 定义 capability / status / reset / queue base / doorbell / irq / fault / perf counter 寄存器窗口。
- [ ] **步骤 2：** 把 AI accelerator 作为独立设备接进 `Machine`，默认不影响现有 board profile。
- [ ] **步骤 3：** 先接 submission / completion ring skeleton、doorbell、token 流转与中断骨架，不提前耦合 compute engine。
- [ ] **步骤 4：** 在 debug snapshot / protocol 里新增只读控制面观测：queue depth、doorbell count、last fault、completion count。
- [ ] **步骤 5：** 运行：
...
- [ ] **步骤 3：** 明确并测试 `scratchpad_overflow`、DMA fault、非法 tile shape 与 partial transfer fail-closed 语义。
- [ ] **步骤 4：** 在 host smoke 中锁住“先搬运、后计算、再回写”的最小数据面闭环。
- [ ] **步骤 5：** 运行：
...
- [ ] **步骤 3：** `v1` 只要求代表性闭环，而不是所有 op × 所有 dtype 铺满；先锁住：
  - 一条 quantized `CNN`
  - 一条 semi-precision `GEMM / matmul-family`
- [ ] **步骤 4：** 在 host smoke 中固化 `CNN` 和 `GEMM / Transformer-like` 两类固定 workload 的输出与 fault 行为。
- [ ] **步骤 5：** 运行：
...
- [ ] **步骤 3：** 给 host 侧入口补最小 profile / summary 输出，至少能看到 package、bytes moved、retired ops 和基本 fault/progress。
- [ ] **步骤 4：** 补 host smoke，锁住 packaging 输出与 profile 行为的兼容性。
- [ ] **步骤 5：** 运行：
...
- [ ] **步骤 3：** 在 debug snapshot / CLI 中补 AI accelerator 只读观测：queue depth、DMA bytes、scratchpad occupancy、engine busy、last fault。
