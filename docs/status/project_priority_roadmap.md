...
- 当前 D 线已经整合进主线，并把 `execution_profile_smoke` 接进默认 `make test` / `make test-pipeline`；`C1 / P4-prep-2` 也已把 shadow-cache 观测接到 `execution_profile`、debug JSON 和 probe 摘要，并由 `vector_cnn_smoke` 锁住第一组 pipeline workload RAM shadow-cache 信号。当前 follow-up 又已把 `functional` backend 的最小 profile 观测接通，并继续纳入 `LR/SC/AMO` atomic traffic，因此默认 `run-workload-xv6` 已能稳定输出更完整的非零 `profile / shadow-cache`，`xv6_boot_smoke` 已锁住当前 functional 5000-step baseline，`test-host-run_debug_cli_probe` 也已锁住真实 probe summary；这两条 xv6 observation guardrail 现已进入默认 `make test` / `make test-pipeline`。下一步更应该先用这条 stable functional observation path 继续守住 workload guardrail，再判断是否值得单独收口 `xv6 / Linux` pipeline gap，而不是反向抢占 A/B/C 的 contract ownership。
