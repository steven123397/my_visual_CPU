...
- `2026-04-27` 同日也补上一层更窄的 fourth-stage prep：repo-generated third-stage ELF `/post-init-exec-smoke` 现在会先显式导出 `stage=execve-fourth-stage`，再 handoff 到新的 repo-generated fourth-stage ELF `/post-init-cleanup-smoke`；该 fourth-stage 会对前一阶段刚重建的空目录再次执行 `unlinkat(AT_REMOVEDIR)`，并要求父目录项再次消失，通过 `stage=unlinkat-reused-dirent-gone` 与 `stage=fourth-stage-reached` 对外暴露最小目录生命周期延伸合同。当前工作区仍缺外部 Linux `Image`，因此这一步本轮只完成了 artifact / host gate 收口；不过同一轮也已补上一条显式的可选 runtime guardrail `make test-host-run_debug_cli_probe_linux_proto_runtime`，后续只要把 `Image` 放到默认位置，就能直接重跑这条 fourth-stage runtime freeze。
