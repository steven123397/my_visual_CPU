...
- D 线的 `execution_profile`、debug CLI profile 导出、面向 `memory_region` 的读侧观测合同，以及 `C1 / P4-prep-2` 的 `shadow_cache` 观测面都已进入主线；`test-host-execution_profile_smoke` 已补进默认 `make test` / `make test-pipeline` guardrail。
