...
- `2026-04-26` 又把这条 block-rootfs second-stage userland baseline 继续往前推进三刀：repo-generated `/post-init-smoke` 现已先补上最小 `clone3/fork-like -> child write -> parent wait4` process lifecycle smoke，并通过 `mycpu linux userland: stage=fork-child-wrote` 与 `stage=parent-wait4-ok` 暴露到 UART；随后又补上一颗 repo-generated third-stage ELF `/post-init-exec-smoke`，把 second-stage 继续推进到 `stage=execve-third-stage` 并稳定 `execve()` 到 `stage=third-stage-reached`；同日 third-stage 也再补上 `mkdir -> chdir -> create/write/read/unlink nested file` 的最小目录/路径解析 smoke，并通过 `stage=mkdir-chdir-ok` 与 `stage=nested-file-roundtrip-ok` 暴露到 UART。基于本地 `CONFIG_RISCV_ISA_C=y` Linux `Image`，当前 repo-generated block-rootfs 路径会在 `stage=file-readable`、`stage=rootfs-rw-roundtrip-ok` 之后继续稳定命中 `stage=fork-child-wrote`、`stage=parent-wait4-ok`、`stage=execve-third-stage`、`stage=mkdir-chdir-ok`、`stage=nested-file-roundtrip-ok`、`stage=third-stage-reached` 与 `post-init reached`；因此活跃 blocker 已进一步收窄到 multi-stage post-init exec + path-resolution 之后的下一处更后 userland checkpoint。
