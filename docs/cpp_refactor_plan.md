...
下一阶段不应直接在当前参考执行器路径上继续把特权、MMU、设备平台、后续执行后端全部叠加到 `cpu.cpp` 中。更合理的做法是先完成一次有结构收益的 C++ 重构，建立清晰的模块边界和状态归属，为后续 Phase 1 的 OS bring-up 功能打地基。
...
当前仓库已经完成了这一阶段的第一步：`CPU` 状态已拆成第一版 `CoreState + CsrFile`，并保持现有参考执行路径和汇编回归可运行。接下来的重点不再是“是否拆状态”，而是继续把 trap 和后续语义边界从 `cpu_step()` 周围收紧。
...
**保持现有 `Machine + Bus + Ram + CoreState + CsrFile` 参考路径不变，下一步把 trap 逻辑从当前 `cpu_step()` 周边继续收敛成更明确的 `TrapController` 边界。**

原因：

- `Machine + Bus + Ram + CoreState + CsrFile` 这一步已经具备基本结构收益
- trap 入口、返回和中断路由仍然是下一块最需要独立测试和收口的边界
