...
- At the current repository state, describe `Machine` / `Bus` / `Ram`, explicit `Uart16550` / `Clint` / minimal `Plic` / host-backed block-oriented `SimpleStorage` devices, the shared MMIO platform contract header/doc, explicit `ElfLoader` / `BinaryLoader` loader boundaries, the `CoreState + CsrFile` split, the `TrapController` boundary, the `AddressSpace` boundary with Sv39 virtual memory support, a minimal TLB, edge-fault coverage, and minimal `SUM` / `MXR` handling, plus the guest-side VM/trap layer with reusable VM-owned page-fault policy/handling, stricter explicit guest VM map/unmap semantics with VM-enabled auto local-TLB maintenance, explicit current-phase kernel/user address-window helpers, the kernel-global versus user-owned VM ownership split, the first explicit `vm_user_region_t + vm_object_t` split between virtual region metadata and backing objects, the first explicit `vm_address_space_t` / `vm_process_t` object layer for the current single-user path with process-owned user-region registration and per-address-space fault-action policy state, the first explicit guest-side `trap_context_t` object layer with active trap-context tracking and standard supervisor-timer / supervisor-external / user-`ecall` resume policy helpers, the first explicit trap-side `trap_user_runtime_t` object layer for bound process/trap-context ownership, delegated user entry arguments, timer signal state, user-`ecall` resume validation, runtime-owned supervisor trap-stack configuration, and shared U-mode enter/resume glue, explicit region-object clear/rebind semantics, explicit process-side user-region removal/reset plus address-space disable/destroy lifecycle cleanup, lifecycle smoke coverage around address-space reuse and page-table/page-pool reclamation, and the first real U-mode bring-up path with real user stack/trap-stack ownership and delegated timer return as already completed incremental steps; describe broader user interrupt coverage, expanded CSR coverage, richer user-memory objects, and further device-platform targets beyond the minimal storage path as the next structural targets for OS bring-up, and describe graphical CPU visualization as a post-Phase-1 discussion item rather than current execution work.
