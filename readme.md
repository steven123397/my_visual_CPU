...
当前处于从 C 原型向模块化 C++ 架构迁移的早期阶段。现有功能路径仍以原始参考语义为主，但已经具备更完整的 Phase 1 OS bring-up 地基：裸机程序执行、UART/CLINT/PLIC/MMIO block storage 平台、M/S/U 特权路径、Sv39 虚拟内存，以及一个带 early allocator、最小 PMM、guest-side Sv39 页表层、VM-owned page-fault policy/handling、显式 kernel/user 地址窗口 helper、kernel-global 映射跟踪、`vm_user_region_t + vm_object_t` 分层、physical + anonymous user-memory object backing、region-to-object offset/window binding、object-owned page resolution / attachment-aware reset 语义、第一批显式 `vm_address_space_t` / `vm_process_t` 对象层、process-owned user-region 注册、batch user-region/object binding helper、per-address-space fault-action policy 状态、显式 object-owned user mapping/fault-backing 流程、显式 region-object clear/rebind 语义、process-side user-region remove/reset 与 address-space disable/destroy 生命周期、第一批显式 `trap_context_t` 对象层、标准 supervisor-timer / supervisor-external / user-`ecall` resume policy helper、第一批显式 `trap_user_runtime_t` 对象层、runtime-owned supervisor trap-stack 配置、timer/external user-signal 状态、formal runtime re-prepare/reset-unbind plus destroy/reset cleanup 语义、explicit deactivate-to-supervisor-only switching、标准 user-runtime prepare helper、共享的 U-mode enter/resume 汇编桥、一个把 `vm_address_space_t + vm_process_t + trap_user_runtime_t` 收拢起来的顶层 `user_task_t` 对象层、一个继续把标准单用户地址布局、exec/stack/alias/anon object 组装、reset 生命周期和标准准备路径收起来的 `user_task_bootstrap_t` 对象层、一个再往上一层收拢 planned standard user image + task/bootstrap lifecycle、具名标准段访问、具名 region/object map/fault-bind/unmap/reset helper 和标准 create/prepare/activate/enter/deactivate 路径的 `user_program_t` 对象层、一个继续把 `supervisor_demo` 剩余 planned-layout/create-shape、lifecycle、address-space、runtime-prepare、activation、active-memory/fault、user-entry、supervisor-only deactivation、invalid-region/remap-region、fault-range 和 fault-action smoke orchestration 收起来的 `user_program_smoke_t` helper、以及一个继续把 demo 侧 early memory-layout/PMM/page-allocation smoke、active-phase defaults/page priming、trap callback state、user-return validation、完整 platform-tail storage/interrupt smoke 和 runtime re-prepare/destroy-cleanup validation 收起来的 `supervisor_demo_smoke` helper，以及独立 guest runtime-context 模块、active process/address-space/trap-context 激活语义和 trap-to-VM page-fault context 传递。
...
`make test` 会构建汇编样例和最小 guest supervisor demo，并校验 UART 输出是否与预期一致；单个样例异常卡死时会超时失败。当前除综合回归外，还包含 `loads_signed_unsigned`、`alu_word`、`branches_signed_unsigned`、`muldiv`、`fence_noop` 这类更细粒度的指令族回归，以及 `privilege_transitions`、`sret_transitions`、`supervisor_exception_delegation`、`supervisor_timer_interrupt`、`csr_access_control`、`access_faults`、`csr_semantic_consistency` 这类特权/异常/CSR 语义一致性回归，`plic_*` / `storage_device_basic` / `supervisor_platform_smoke` / `clint_split_access` 这类平台回归，`sv39_*` 这类虚拟内存/TLB 回归，以及覆盖 guest-side demand paging、recoverable fault policy、VM 已启用后显式 guest VM map/unmap 路径自动维护本地 TLB 一致性、当前 user `< MEM_BASE` / kernel `[MEM_BASE, MEM_BASE + MEM_SIZE)` 地址窗口 helper、kernel-global 与 user-owned VM ownership split、`vm_user_region_t + vm_object_t` 分层、physical/anonymous `vm_object_t` backing、region-to-object offset/window binding、object-owned fault resolution、attachment-aware object reset、anonymous-object lazy fault-in/remap persistence、multi-window object reuse、第一批显式 `vm_address_space_t` / `vm_process_t` 生命周期、batch user-region/object binding helper、显式 `trap_context_t` 激活与注册式 handler state、标准 supervisor-timer / supervisor-external / user-`ecall` resume policy helper、显式 `trap_user_runtime_t` 绑定/激活/进入 U-mode 路径、runtime-owned supervisor trap-stack 配置、timer/external user-signal 状态、formal runtime re-prepare/reset-unbind plus destroy/reset cleanup 语义、explicit deactivate-to-supervisor-only switching、标准 user-runtime prepare helper、顶层 `user_task_t` 对象的 create/prepare/activate/enter/deactivate 路径、`user_task_bootstrap_t` 对标准单用户布局/object 组装/bind/prepare/reset 的封装、`user_program_t` 对 planned standard user image、具名标准 region/object helper 与 task/bootstrap create/prepare/activate/enter/deactivate 路径的更高层封装、`user_program_smoke_t` 对剩余 planned-layout/create-shape、lifecycle、address-space、runtime-prepare、activation、active-memory/fault、user-entry、supervisor-only deactivation、invalid-region/remap-region、fault-range 和 fault-action smoke orchestration 的封装、`supervisor_demo_smoke` 对 demo-side early memory-layout/PMM/page-allocation smoke、active-phase defaults/page priming、trap callback state、user-return validation、完整 platform-tail storage/interrupt smoke 和 explicit runtime re-prepare/destroy-cleanup validation 的封装、共享 enter/resume 汇编桥、显式 region-object clear/rebind、process-owned user-region remove/reset、address-space disable/destroy、address-space reuse 与 page-table/page-pool reclamation smoke，以及真实 U-mode 执行、delegated user page fault、delegated user `ecall`、delegated supervisor timer/external interrupt return、runtime re-prepare clean-state、destroy/reset cleanup 和 post-return supervisor-only fallback 语义与 user stack / trap stack 切换的 `guest_supervisor_demo` bring-up 回归。
...
- 最小 guest supervisor platform layer、统一 trap dispatch、注册式 interrupt/exception handler、第一批显式 `trap_context_t` 对象层、标准 supervisor-timer / supervisor-external / user-`ecall` resume policy helper、第一批显式 `trap_user_runtime_t` 对象层、runtime-owned supervisor trap-stack 配置、timer/external user-signal 状态、formal runtime re-prepare/reset-unbind plus destroy/reset cleanup 语义、explicit deactivate-to-supervisor-only switching、标准 user-runtime prepare helper、共享 U-mode enter/resume 汇编桥、顶层 `user_task_t` 对象层、继续把标准单用户地址布局、exec/stack/alias/anon object 组装和准备路径收起来的 `user_task_bootstrap_t` 对象层、再往上一层收拢 planned standard user image、具名标准 region/object helper 与 task/bootstrap lifecycle 的 `user_program_t` 对象层、以及继续把 `supervisor_demo` 剩余 planned-layout/create-shape、lifecycle、address-space、runtime-prepare、activation、active-memory/fault、user-entry、supervisor-only deactivation、invalid-region/remap-region、fault-range 和 fault-action smoke orchestration 收起来的 `user_program_smoke_t` helper、以及继续把 demo-side early memory-layout/PMM/page-allocation smoke、active-phase defaults/page priming、trap callback state、user-return validation、完整 platform-tail storage/interrupt smoke 和 explicit runtime re-prepare/destroy-cleanup validation 收起来的 `supervisor_demo_smoke` helper、VM-owned page-fault policy/handling，以及 linker-backed early allocator + bitmap-backed PMM + guest-side Sv39 page-table builder + kernel-global 映射跟踪 + `vm_user_region_t + vm_object_t` 分层 + physical/anonymous `vm_object_t` backing + region-to-object offset/window binding + object-owned page resolution / attachment-aware reset + 第一批显式 `vm_address_space_t` / `vm_process_t` 对象层 + process-owned user-region 注册 + batch user-region/object binding helper + per-address-space fault-action policy + 显式 region-object clear/rebind + process-side user-region remove/reset + address-space disable/destroy lifecycle cleanup + recoverable fault policy registration + page-granular kernel mapping / fault handling + VM 启用后的 map/unmap 本地 TLB 同步 + 当前 user `< MEM_BASE` / kernel `[MEM_BASE, MEM_BASE + MEM_SIZE)` split helper + 真实 U-mode 执行、delegated trap return、real user stack 和 dedicated supervisor trap stack 的 bring-up 验证
- `ecall` a7=93 退出约定
...
- `kernel/`：`console` / `storage` / `timer` / `trap` / `memory` / `pmm` / `vm` / `runtime_context` / `user_task` / `user_task_bootstrap` / `user_program` / `user_program_smoke` / `supervisor_demo_smoke` 这些最小 guest 侧模块实现，当前已覆盖页粒度 kernel mapping、VM-owned page-fault dispatch/policy、kernel-global 映射跟踪、`vm_user_region_t + vm_object_t` 分层、physical/anonymous `vm_object_t` backing、region-to-object offset/window binding、object-owned page resolution、attachment-aware object reset、第一批显式 `vm_address_space_t` / `vm_process_t` 生命周期、batch user-region/object binding helper、第一批显式 `trap_context_t` 生命周期、第一批显式 `trap_user_runtime_t` 生命周期、runtime-owned supervisor trap-stack 配置、timer/external user-signal 状态、formal runtime re-prepare/reset-unbind plus destroy/reset cleanup 语义、explicit deactivate-to-supervisor-only switching、标准 user-runtime prepare helper、顶层 `user_task_t` 生命周期与 create/prepare/activate/enter/deactivate helper、`user_task_bootstrap_t` 的标准单用户布局/object 组装/bind/prepare/reset helper、`user_program_t` 的 planned standard user image、具名标准 region/object map/fault-bind/unmap/reset helper 与 task/bootstrap create/prepare/activate/enter/deactivate helper、`user_program_smoke_t` 对 supervisor-demo 剩余 planned-layout/create-shape、lifecycle、address-space、runtime-prepare、activation、active-memory/fault、user-entry、supervisor-only deactivation、invalid-region/remap-region、fault-range 和 fault-action smoke orchestration 的封装、`supervisor_demo_smoke` 对 demo-side early memory-layout/PMM/page-allocation smoke、active-phase defaults/page priming、trap callback state、user-return validation、完整 platform-tail storage/interrupt smoke 和 explicit runtime re-prepare/destroy-cleanup validation 的封装、共享 U-mode enter/resume 汇编桥、标准 supervisor-timer / supervisor-external / user-`ecall` resume policy helper、process-owned user-region 注册、per-address-space fault-action policy、显式 object-owned user mapping/fault-backing 流程、显式 region-object clear/rebind、process-side user-region remove/reset、address-space disable/destroy、独立 active process/address-space/trap-context runtime context、可恢复 page-fault 注册、较严格的显式 guest VM map/unmap 语义、VM 启用后成功 map/unmap 自动维护本地 TLB 一致性、当前 user `< MEM_BASE` / kernel `[MEM_BASE, MEM_BASE + MEM_SIZE)` 地址窗口 helper，以及 `SUM` 驱动的 supervisor-side user-page 访问辅助。
