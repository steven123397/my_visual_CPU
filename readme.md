...
- `kernel_alpha/`：独立 kernel alpha bring-up 入口，当前用于验证第一次真正的小 kernel alpha 的 boot / PMM / Sv39 / PLIC/UART external interrupt / timer interrupt / storage readiness probe / storage read 最小基线；其中 `common.c` 已收口 alpha 专属的 PLIC / first delivery / storage probe phase helper。
