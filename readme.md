...
当前处于从 C 原型向模块化 C++ 架构迁移的早期阶段。现有功能路径仍以原始参考语义为主，但已经具备更完整的 Phase 1 OS bring-up 地基：裸机程序执行、UART/CLINT/PLIC/MMIO block storage 平台、M/S/U 特权路径、Sv39 虚拟内存，以及一个带 early allocator、最小 PMM、guest-side Sv39 页表层、VM-owned page-fault policy/handling、显式 kernel/user 地址窗口 helper、kernel-global 映射跟踪、`vm_user_region_t + vm_object_t` 分层、第一批显式 `vm_address_space_t` / `vm_process_t` 对象层、process-owned user-region 注册、per-address-space fault-action policy 状态、显式 object-owned user mapping/fault-backing 流程、显式 region-object clear/rebind 语义、process-side user-region remove/reset 与 address-space disable/destroy 生命周期、第一批显式 `trap_context_t` 对象层、标准 supervisor-timer / supervisor-external / user-`ecall` resume policy helper、第一批显式 `trap_user_runtime_t` 对象层、runtime-owned supervisor trap-stack 配置、共享的 U-mode enter/resume 汇编桥，以及独立 guest runtime-context 模块、active process/address-space/trap-context 激活语义和 trap-to-VM page-fault context 传递。
...
- 最小 guest supervisor platform layer、统一 trap dispatch、注册式 interrupt/exception handler、第一批显式 `trap_context_t` 对象层、标准 supervisor-timer / supervisor-external / user-`ecall` resume policy helper、第一批显式 `trap_user_runtime_t` 对象层、runtime-owned supervisor trap-stack 配置、共享 U-mode enter/resume 汇编桥、VM-owned page-fault policy/handling，以及 linker-backed early allocator + bitmap-backed PMM + guest-side Sv39 page-table builder + kernel-global 映射跟踪 + `vm_user_region_t + vm_object_t` 分层 + 第一批显式 `vm_address_space_t` / `vm_process_t` 对象层 + process-owned user-region 注册 + per-address-space fault-action policy + 显式 region-object clear/rebind + process-side user-region remove/reset + address-space disable/destroy lifecycle cleanup + recoverable fault policy registration + page-granular kernel mapping / fault handling + VM 启用后的 map/unmap 本地 TLB 同步 + 当前 user `< MEM_BASE` / kernel `[MEM_BASE, MEM_BASE + MEM_SIZE)` split helper + 真实 U-mode 执行、delegated trap return、real user stack 和 dedicated supervisor trap stack 的 bring-up 验证
- `ecall` a7=93 退出约定
