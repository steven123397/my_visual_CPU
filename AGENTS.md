...
- Sv39 virtual memory is now implemented with 3-level page table walk, a minimal TLB, permission checks, and page fault handling. On the guest/runtime side, a minimal PMM, guest-side Sv39 page-table builder, S-mode page-fault dispatch, reusable guest VM/trap page-fault handling, stricter `vm_map_range` / `vm_unmap_page` semantics, and first kernel/user range-split helpers are now landed. The immediate next functional step is to continue that fault-driven address-space evolution: move beyond the current demo-scoped fault registrations toward cleaner VM-owned policy, broaden reusable fault/mapping management, and keep preparing a cleaner kernel/user address-space split for OS bring-up.
...
- A shared MMIO platform contract now exists in code/docs, along with a reusable guest-side platform library, an assembly shim consumed by tests, and a first minimal supervisor demo that exercises storage, supervisor external interrupts, supervisor timer interrupts, registered trap handlers through a unified dispatch path, a dedicated guest-side page-fault handler path, a linker-backed early allocator plus bitmap-backed physical page manager, and guest-side Sv39 page-table bring-up with fault-range-backed page-fault handling, stricter `vm_map_range` / `vm_unmap_page` semantics, page-granular kernel mappings, and first kernel/user range-split helpers from S-mode
