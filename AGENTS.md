...
- 把真实 `virtio-blk` board path 下的 `xv6` shell 守成稳定 guardrail，并在现有 `flat/payload/set_gpr` 与 `linux_proto` foundation 之上推进真实 Linux 资产、板级 `DTB/chosen/cmdline` 与首个 boot checkpoint。
