...
- Sv39 virtual memory is now implemented with 3-level page table walk, a minimal TLB, permission checks, and page fault handling. On the guest/runtime side, a minimal PMM, guest-side Sv39 page-table builder, S-mode page-fault dispatch, reusable guest VM/trap page-fault handling, VM-owned page-fault policy registration for demand-mapped and recoverable faults, stricter `vm_map_range` / `vm_unmap_page` semantics with VM-enabled auto local-TLB maintenance, explicit current-phase kernel/user address-window helpers, and a first formal split between kernel-global mappings and user-owned regions are now landed. The immediate next functional step is to turn those user-owned regions into more explicit user VM objects and begin real U-mode bring-up on top of that split.
