...
- A first `ExecutionBackend` split now exists, with `FunctionalBackend` preserving the simple reference path and `PipelineBackend` providing an incremental alternate execution model selectable from `Machine` / CLI
- Shared `InstructionSemantics + InsnEffects + ExecutionContext` objects now provide a common semantic source for the functional and pipeline backends
- `AddressSpace` now exposes explicit fault-return fetch/load/store result APIs for backend-controlled trap timing while preserving the legacy trap-producing wrappers for the reference path
- A first incremental 5-stage pipeline backend now supports the current implemented instruction/privilege subset through a real `IF/ID/EX/MEM/WB` path with EX/MEM plus MEM/WB operand forwarding, CSR visible-value projection, single-bubble `load-use` interlocks, EX-stage redirect/flush, MEM-stage memory access, and WB-stage GPR/CSR/trap-return commit, plus commit-boundary interrupt/fault handling across the current validated asm suite
...
- host-side regression coverage for backend CLI selection, shared instruction semantics, address-space fault results, pipeline backend smoke scenarios, and commit-level `functional` / `pipeline` backend differential smoke
- Assembly regression tests with output checking for:
  ...
  - a first shared guest-side supervisor platform layer plus smoke/demo coverage that consumes the shared MMIO contract for UART output, supervisor PLIC setup/claim/complete, supervisor timer handling through CLINT `mtime` / `mtimecmp`, block-0 storage reads, and a minimal C-based supervisor runtime with unified trap dispatch, registered interrupt/exception handlers, a first explicit guest-side `trap_context_t` object plus active trap-context tracking and standard supervisor-timer / supervisor-external / user-`ecall` resume policy helpers, a first explicit trap-side `trap_user_runtime_t` object for bound process/trap-context ownership, delegated user entry arguments, timer/external signal state, formal runtime re-prepare/reset-unbind plus destroy/reset cleanup semantics, explicit deactivate-to-supervisor-only switching, user-`ecall` resume validation, runtime-owned supervisor trap-stack configuration, shared U-mode enter/resume glue, and a standard helper that composes user-context install, runtime prepare, and standard trap-policy wiring, a small top-level `user_task_t` object layer that now owns the current single-user address-space/process/runtime lifecycle and funnels the standard create/prepare/activate/enter/deactivate path, plus a `user_task_bootstrap_t` layer that further owns the current standard single-user address layout, exec/stack/alias/anon object assembly, process-region binding, reset semantics, and standard runtime-prepare path, plus a new `user_program_t` layer above that which owns the planned standard user-image/task/bootstrap lifecycle, named standard segment access, named standard region/object map/fault-bind/unmap/reset helpers, and the standard create/prepare/activate/enter/deactivate path, plus a small `user_program_smoke_t` helper that now owns the remaining supervisor-demo planned-layout/create-shape, lifecycle, address-space, runtime-prepare, activation, active-memory/fault, user-entry, supervisor-only deactivation, invalid-region/remap-region, fault-range, and fault-action orchestration around fault-skip/fault-resume smoke, plus a small `supervisor_demo_smoke` helper that now owns demo-side early memory-layout/PMM/page-allocation smoke, active-phase defaults/page priming, trap callback state, user-return validation, full platform-tail storage/interrupt smoke, and explicit runtime re-prepare/destroy-cleanup validation, a guest-side VM-owned page-fault policy path for demand-mapped and recoverable faults, explicit linker-defined memory layout, an early bump allocator, a bitmap-backed physical page manager, and a minimal guest-side Sv39 page-table builder with page-granular kernel mappings, explicit kernel-global mapping tracking, user-owned region ownership, a first explicit `vm_user_region_t + vm_object_t` split between virtual region metadata and backing objects, a first richer `vm_object_t` backing model with physical objects plus anonymous user-memory objects, region-to-object offset/window binding, object-owned page resolution on fault, attachment-aware reset semantics, and batch process-side user-region/object binding helpers, a first explicit `vm_address_space_t` / `vm_process_t` object layer for the current single-user path with process-owned user-region registration and per-address-space fault-action policy state, explicit object-owned user mapping/fault-backing flow, explicit region-object clear/rebind semantics, explicit process-side user-region removal/reset and address-space disable/destroy lifecycle paths, a small shared guest runtime-context module for the active process/address-space and active trap context, explicit active-process/address-space activation for the running context, explicit trap-to-VM page-fault context passing, and a first real U-mode bring-up path with real user stack, dedicated supervisor trap stack, delegated user page-fault recovery, delegated user `ecall`, delegated timer/external-interrupt return back to U-mode, and explicit post-return supervisor-only fallback
...
- A broader future pipeline model for project features that are not yet implemented anywhere in the repository, such as wider privileged/CSR coverage beyond the current validated subset, more realistic variable-latency memory timing, cache-aware timing, and later advanced microarchitecture
- Cache, multicore, or out-of-order models
...
- A first backend split and shared semantic layer are now landed. On the backend side, the immediate next incremental step is to continue filling out the pipeline backend, especially forwarding, load-use handling, CSR/system coverage, and precise commit-side trap behavior.
- Sv39 virtual memory is now implemented with 3-level page table walk, a minimal TLB, permission checks, and page fault handling. On the guest/runtime side, a minimal PMM, guest-side Sv39 page-table builder, S-mode page-fault dispatch, reusable guest VM/trap page-fault handling, stricter explicit guest VM map/unmap semantics with VM-enabled auto local-TLB maintenance, VM-owned page-fault policy registration for demand-mapped and recoverable faults, explicit current-phase kernel/user address-window helpers, a formal split between kernel-global mappings and user-owned regions, a first explicit `vm_user_region_t + vm_object_t` split between virtual region metadata and backing objects, a first richer `vm_object_t` backing model with physical objects plus anonymous user-memory objects, region-to-object offset/window binding, object-owned page resolution on fault, attachment-aware reset semantics, and batch process-side user-region/object binding helpers, a first explicit `vm_address_space_t` / `vm_process_t` object layer for the current single-user path with process-owned user-region registration and per-address-space fault-action policy state, explicit object-owned user mapping/fault-backing flow, explicit region-object clear/rebind semantics, explicit process-side user-region removal/reset and address-space disable/destroy lifecycle cleanup, a first explicit guest-side `trap_context_t` object layer with active trap-context tracking plus standard supervisor-timer / supervisor-external / user-`ecall` resume policy helpers, a first explicit trap-side `trap_user_runtime_t` object layer for bound process/trap-context ownership, delegated user entry arguments, timer/external signal state, formal runtime re-prepare/reset-unbind plus destroy/reset cleanup semantics, explicit deactivate-to-supervisor-only switching, user-`ecall` resume validation, runtime-owned supervisor trap-stack configuration, shared U-mode enter/resume glue, and a standard helper that composes user-context install, runtime prepare, and standard trap-policy wiring, plus a small top-level `user_task_t` object layer that owns the current single-user address-space/process/runtime lifecycle and standard create/prepare/activate/enter/deactivate path, a `user_task_bootstrap_t` object layer that now owns the current standard single-user address layout, exec/stack/alias/anon object assembly, process-region binding, reset semantics, and standard runtime-prepare path, a new `user_program_t` layer above that which owns the planned standard user-image/task/bootstrap lifecycle plus named standard segment access, named standard region/object map/fault-bind/unmap/reset helpers, and the standard create/prepare/activate/enter/deactivate path, and a small `user_program_smoke_t` helper that now owns the remaining supervisor-demo planned-layout/create-shape, lifecycle, address-space, runtime-prepare, activation, active-memory/fault, user-entry, supervisor-only deactivation, invalid-region/remap-region, fault-range, and fault-action orchestration around fault-skip/fault-resume smoke, plus a small `supervisor_demo_smoke` helper that now owns demo-side early memory-layout/PMM/page-allocation smoke, active-phase defaults/page priming, trap callback state, user-return validation, full platform-tail storage/interrupt smoke, and explicit runtime re-prepare/destroy-cleanup validation are now landed, along with a small shared guest runtime-context module for the active process/address-space and active trap context, explicit active-process/address-space activation for the running context, explicit trap-to-VM page-fault context passing, and a first real U-mode bring-up path with real user stack/trap-stack ownership, delegated timer/external return, and explicit post-return supervisor-only fallback. The immediate next functional step is to use that path to expose and close the remaining Phase 1 gaps, especially around further process/runtime refinement and any additional user interrupt classes needed beyond the current timer-plus-external signal path on top of the newly landed physical-plus-anonymous user-memory object path plus the new batch region-binding/runtime-prepare/user-task/bootstrap/user-program helpers.
- A future graphical or real-time visual CPU state display is explicitly deferred until after Phase 1 is stable and complete. It is a valid later task, but it is not part of the current bring-up critical path and should not displace Phase 1 simulator/OS work.
...
- A shared MMIO platform contract now exists in code/docs, along with a reusable guest-side platform library, an assembly shim consumed by tests, and a first minimal supervisor demo that exercises storage, supervisor external interrupts, supervisor timer interrupts, registered trap handlers through a unified dispatch path, a first explicit guest-side `trap_context_t` object plus active trap-context tracking and standard supervisor-timer / supervisor-external / user-`ecall` resume policy helpers, a first explicit trap-side `trap_user_runtime_t` object for bound process/trap-context ownership, delegated user entry arguments, timer/external signal state, formal runtime re-prepare/reset-unbind plus destroy/reset cleanup semantics, explicit deactivate-to-supervisor-only switching, user-`ecall` resume validation, runtime-owned supervisor trap-stack configuration, shared U-mode enter/resume glue, and a standard helper that composes user-context install, runtime prepare, and standard trap-policy wiring, plus a small top-level `user_task_t` object layer that now owns the current single-user address-space/process/runtime lifecycle and standard create/prepare/activate/enter/deactivate path, a `user_task_bootstrap_t` object layer that further owns the current standard single-user address layout, exec/stack/alias/anon object assembly, process-region binding, reset semantics, and standard runtime-prepare path, a new `user_program_t` layer above that which owns the planned standard user-image/task/bootstrap lifecycle plus named standard segment access, named standard region/object map/fault-bind/unmap/reset helpers, and the standard create/prepare/activate/enter/deactivate path, and a small `user_program_smoke_t` helper that now owns the remaining supervisor-demo planned-layout/create-shape, lifecycle, address-space, runtime-prepare, activation, active-memory/fault, user-entry, supervisor-only deactivation, invalid-region/remap-region, fault-range, and fault-action orchestration around fault-skip/fault-resume smoke, plus a small `supervisor_demo_smoke` helper that now owns demo-side early memory-layout/PMM/page-allocation smoke, active-phase defaults/page priming, trap callback state, user-return validation, full platform-tail storage/interrupt smoke, and explicit runtime re-prepare/destroy-cleanup validation, a VM-owned guest-side page-fault policy path, a linker-backed early allocator plus bitmap-backed physical page manager, and guest-side Sv39 page-table bring-up with kernel-global mapping tracking, user-owned region ownership, a first explicit `vm_user_region_t + vm_object_t` split between virtual region metadata and backing objects, a first richer `vm_object_t` backing model with physical objects plus anonymous user-memory objects, region-to-object offset/window binding, object-owned page resolution on fault, attachment-aware reset semantics, and batch process-side user-region/object binding helpers, a first explicit `vm_address_space_t` / `vm_process_t` object path for the current single-user bring-up flow with process-owned user-region registration and per-address-space fault-action policy state, explicit region-object clear/rebind semantics, explicit process-side user-region removal/reset and address-space disable/destroy lifecycle cleanup, a small shared guest runtime-context module for the active process/address-space and active trap context, recoverable page-fault policy registration, lifecycle smoke coverage around address-space reuse and page-table/page-pool reclamation, a first real U-mode entry/return path through delegated traps with real user stack plus dedicated supervisor trap stack, stricter explicit guest VM map/unmap semantics with VM-enabled auto local-TLB maintenance, page-granular kernel mappings, and explicit current-phase kernel/user address-window helpers from S-mode
...
- A first `ExecutionBackend` abstraction now exists with `FunctionalBackend` plus an incremental `PipelineBackend`, and shared `InstructionSemantics` / `InsnEffects` now serve as the common semantic source across backends
- `AddressSpace` now also provides explicit fault-return fetch/load/store helpers for backend-controlled trap timing
...
- Keep the reference execution path simple; now that a concrete second execution model exists, keep the backend abstraction scoped to real backend needs rather than speculative layering.
...
- At the current repository state, describe `Machine` / `Bus` / `Ram`, explicit `Uart16550` / `Clint` / minimal `Plic` / host-backed block-oriented `SimpleStorage` devices, the shared MMIO platform contract header/doc, explicit `ElfLoader` / `BinaryLoader` loader boundaries, the `CoreState + CsrFile` split, the `TrapController` boundary, the `AddressSpace` boundary with Sv39 virtual memory support, a minimal TLB, edge-fault coverage, and minimal `SUM` / `MXR` handling, the `ExecutionBackend` split between `FunctionalBackend` and the incremental `PipelineBackend`, the shared `InstructionSemantics` / `InsnEffects` layer, the fault-return `AddressSpace` access-result APIs, and the current staged integer/control-flow/`load-store` pipeline bring-up status, plus the guest-side VM/trap layer with reusable VM-owned page-fault policy/handling, stricter explicit guest VM map/unmap semantics with VM-enabled auto local-TLB maintenance, explicit current-phase kernel/user address-window helpers, the kernel-global versus user-owned VM ownership split, the first explicit `vm_user_region_t + vm_object_t` split between virtual region metadata and backing objects, the first richer `vm_object_t` backing model with physical objects plus anonymous user-memory objects, region-to-object offset/window binding, object-owned page resolution on fault, attachment-aware reset semantics, and batch process-side user-region/object binding helpers, the first explicit `vm_address_space_t` / `vm_process_t` object layer for the current single-user path with process-owned user-region registration and per-address-space fault-action policy state, the first explicit guest-side `trap_context_t` object layer with active trap-context tracking and standard supervisor-timer / supervisor-external / user-`ecall` resume policy helpers, the first explicit trap-side `trap_user_runtime_t` object layer for bound process/trap-context ownership, delegated user entry arguments, timer/external signal state, formal runtime re-prepare/reset-unbind plus destroy/reset cleanup semantics, explicit deactivate-to-supervisor-only switching, user-`ecall` resume validation, runtime-owned supervisor trap-stack configuration, shared U-mode enter/resume glue, and a standard helper that composes user-context install, runtime prepare, and standard trap-policy wiring, plus the top-level `user_task_t` object layer that owns the current single-user address-space/process/runtime lifecycle and standard create/prepare/activate/enter/deactivate path, the `user_task_bootstrap_t` layer that further owns the current standard single-user address layout, exec/stack/alias/anon object assembly, process-region binding, reset semantics, and standard runtime-prepare path, the `user_program_t` layer above that which owns the planned standard user-image/task/bootstrap lifecycle plus named standard segment access, named standard region/object map/fault-bind/unmap/reset helpers, and the standard create/prepare/activate/enter/deactivate path, and the small `user_program_smoke_t` helper that now owns the remaining supervisor-demo planned-layout/create-shape, lifecycle, address-space, runtime-prepare, activation, active-memory/fault, user-entry, supervisor-only deactivation, invalid-region/remap-region, fault-range, and fault-action orchestration around fault-skip/fault-resume smoke, plus a small `supervisor_demo_smoke` helper that now owns demo-side early memory-layout/PMM/page-allocation smoke, active-phase defaults/page priming, trap callback state, user-return validation, full platform-tail storage/interrupt smoke, and explicit runtime re-prepare/destroy-cleanup validation, explicit region-object clear/rebind semantics, explicit process-side user-region removal/reset plus address-space disable/destroy lifecycle cleanup, lifecycle smoke coverage around address-space reuse and page-table/page-pool reclamation, and the first real U-mode bring-up path with real user stack/trap-stack ownership, delegated timer/external return, and explicit post-return supervisor-only fallback as already completed incremental steps; describe further process/runtime refinement, any remaining user interrupt expansion beyond the current timer-plus-external signal path, expanded CSR coverage, fuller pipeline completion, and further device-platform targets beyond the minimal storage path as the next structural targets for OS bring-up, and describe graphical CPU visualization as a post-Phase-1 discussion item rather than current execution work.
